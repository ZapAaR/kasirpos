...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-6 animate-fade-in-up">
        ...
        <div class="flex-1">
            ...
            <p class="text-gray-400 mt-1">Perbarui detail produk</p>
        </div>
        <a href="{{ route('produk.show', $produk) }}"
           class="hidden sm:inline-flex items-center gap-2 px-4 py-2.5 border border-gray-200 rounded-xl hover:bg-gray-50 text-gray-600 transition-colors">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 animate-slide-left">
        <form method="POST" action="{{ route('produk.update', $produk) }}" enctype="multipart/form-data" class="p-6 md:p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="category_id"
                            ...
                            <option value="{{ $kat->id }}" {{ old('category_id', $produk->category_id) == $kat->id ? 'selected' : '' }}>
                                {{ $kat->nama }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-100 rounded-xl p-4">
                        <div class="flex items-center justify-between">
                            ...
                            <p class="text-2xl font-bold text-green-700" id="keuntunganPreview">
                                Rp {{ number_format($produk->harga_jual - $produk->harga_modal, 0, ',', '.') }}
                            </p>
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                        ...
                        <div class="relative">
                            ...
                            <label for="gambarInput"
                                ...
                                <button type="button"
                                        ...
                                        class="absolute top-3 right-3 w-8 h-8 rounded-lg bg-red-500 hover:bg-red-600 text-white flex items-center justify-center shadow-lg z-10">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="bg-gray-50 border border-gray-100 rounded-xl p-4 space-y-3">
                        <h4 class="font-semibold text-gray-700 text-sm">Informasi Produk</h4>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Dibuat</span>
                            <span class="font-medium text-gray-700">{{ $produk->created_at->format('d M Y') }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Terakhir diupdate</span>
                            <span class="font-medium text-gray-700">{{ $produk->updated_at->format('d M Y') }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">ID Produk</span>
                            <span class="font-mono text-gray-700">#{{ str_pad($produk->id, 4, '0', STR_PAD_LEFT) }}</span>
                        </div>
                    </div>
